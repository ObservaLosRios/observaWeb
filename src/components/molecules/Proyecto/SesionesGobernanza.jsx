...
export const SesionesGobernanza = () => {
    ...
    const handleDownload = (url, sessionName) => {
        // Crear un elemento <a> temporal para forzar la descarga
        const link = document.createElement('a');
        link.href = url;
        
        // Intentar obtener el nombre del archivo de la URL o usar uno por defecto
        const filename = url.split('/').pop() || `sesion_gobernanza_${sessionName}.pdf`;
        link.download = filename;
        
        // Atributos importantes para descarga cross-origin
                                            ...
                                            {gobernanzaData.map((session, index) => (
                                                <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors border-b border-gray-200 dark:border-gray-600 last:border-b-0">
                                                    ...
                                                    <td className="px-6 py-4 text-center">
                                                        {session.url ? (
                                                            <button
                                                                onClick={() => handleDownload(session.url, session.tipo_actividad || session.tipo || 'documento')}
                                                                className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-lg focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 transition-colors duration-150 ease-in-out cursor-pointer"
                                                            >
